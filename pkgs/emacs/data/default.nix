...
let
  toLockData =
    ...
    if version == 7 then
      lib.mapAttrs (_: { locked, ... }: locked) nodes
    else
      throw "Unsupported flake.lock version ${version}";

  flakeLockData = if pathExists flakeLockFile then toLockData (lib.importJSON flakeLockFile) else { };

  archiveLockData = if pathExists archiveLockFile then lib.importJSON archiveLockFile else { };

  cachedMetadata =
    if mode == "build" && persistMetadata && pathExists metadataJsonFile then
      lib.importJSON metadataJsonFile
    else
      { };
  ...
  findPrescription =
    revDep: ename:
    packageData.${ename} or (
      if revDep == null then
        throw "Package ${ename} is not found"
      else
        throw "Package ${ename} required by ${revDep} is not found"
    );

  findFromPinned =
    revDep: ename: inventory:
    if hasAttr ename inventory then
      inventory.${ename}
    else if inventory ? _impure then
      inventory._impure.${ename}
    else
      throw "Package ${ename} required by ${revDep} does not exist in the pinned inventory";
  ...
  findPrescription' =
    ...
    if pin == null then
      ...
      findFromPinned revDep ename (
        namedInventories.${pin} or (throw "Inventory named ${pin} does not exist")
      );

  getPackageData0 = import ./package.nix {
    inherit
      lib
      linkFarm
      defaultMainIsAscii
  ...
  toOverrideFn = overrides: if isFunction overrides then overrides else _: _: overrides;
  ...
  getPackageData' =
    ...
    lib.pipe
      ...
      (
        if hasAttr ename inputOverrides then
          (getPackageData revDep ename).extend (toOverrideFn inputOverrides.${ename})
        else
  ...
  go =
    acc: revDeps: enames: ename: data:
    accumPackage (acc // { ${ename} = data; })
      (
        revDeps
      ...
      (
        ...
        ++
          ...
          (lib.subtractLists (builtinLibraries ++ attrNames acc ++ enames) (
  ...
  accumPackage =
    acc: revDeps: enames:
    if length enames == 0 then
      acc
    else if hasAttr (head enames) acc then
      accumPackage acc revDeps (tail enames)
    else
      go acc revDeps enames (head enames)
        ...
        (getPackageData' (revDeps.${head enames} or null) (head enames));
in
accumPackage { } { }
