...
If any of the dependencies are lower than the versions specified in =Package-Requires= header of their dependants, it will exit with non-zero:

#+begin_src sh
  nix build .#emacs-config.depsCheck --print-build-logs
#+end_src
...
#+begin_src nix
  {
    outputs = inputs:
      ...
      in
        {
          checks.x86_64-linux.elisp-versions = emacs-config.depsCheck;
        }
  }
#+end_src
